...
1. [Basic](#basic) - основная задача (обязательна для выполнения)
...
Для проверки работы программы предоставляются входные данные и результаты в виде 6 файлов:
...
**Внимание! Порядок колонок может не совпадать. В обоих файлах есть заголовки.**
...
`.xml` файл содержит в себе следующую структуру:
...
Он должен быть отсортирован по колонке **D1** и содержать данные из всех четырёх файлов.
...
В колонках **MS1**...**MSn** должны находиться суммы значений соответствующих **M1**...**Mn** из 4 файлов сгруппированные
по уникальным значениям комбинаций строк из **D1**...**Dn**.

##### Пример

**Содержимое .tsv файла с данными из 4 файлов:**
